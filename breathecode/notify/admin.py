...
class SlackUserAdmin(admin.ModelAdmin, AdminExportCsvMixin):
    search_fields = [
        'slack_id', 'display_name', 'real_name', 'email', 'user__email', 'user__first_name', 'user__last_name'
    ]
    raw_id_fields = ['user']
    ...
    def user_link(self, obj):
        if obj.user is not None:
            return format_html('<a href="%s">%s</a>' %
                               (reverse('admin:auth_user_change', args=(obj.user.id, )), escape(obj)))
        else:
...
class SlackUserTeamAdmin(admin.ModelAdmin, AdminExportCsvMixin):
    search_fields = [
        'slack_user__email', 'slack_user__user__first_name', 'slack_user__user__last_name', 'slack_team__id',
        'slack_team__name'
    ]
...
class SlackChannelAdmin(admin.ModelAdmin, AdminExportCsvMixin):
    ...
    def cohort_link(self, obj):
        if obj.cohort is not None:
            return format_html('<a href="%s">%s</a>' %
                               (reverse('admin:auth_user_change', args=(obj.cohort.id, )), escape(obj)))
        else:
            return 'No BC cohort'
