...
def mentorship_session_serializer(mentor_profile, mentorship_service, academy, user):
    mentorship_sessions = MentorshipSession.objects.filter(mentor__id=mentor_profile.id,
...
class AuthenticateTestSuite(MentorshipTestCase):
        ...
        for c in statuses:
            ...
            self.assertEqual(self.bc.database.list_of('mentorship.MentorshipSession'), [{
                **self.bc.format.to_dict(model.mentorship_session[0]),
                ...
                'status':
                'FAILED',
                'summary': ('This session expired without assigned mentee, it probably '
    ...
    def test__post__without_mentorship_session__passing_nothing(self):
        ...
        expected = render_post_form(self,
                                    messages=[Message('alert-danger', f'Invalid or expired deliver token.')],
                                    token=model.token,
                                    data=data)
    ...
    def test__post__without_mentorship_session__passing_token(self):
        ...
        expected = render_post_form(self,
                                    messages=[Message('alert-danger', f'Invalid session id.')],
                                    token=model.token,
                                    data=data)
