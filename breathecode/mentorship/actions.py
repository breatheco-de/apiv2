...
def get_or_create_sessions(mentor, mentee=None):

    # mentee is none when the mentor is the one creating the session
    if mentee is None:
        ...
        unfinished_with_mentee_sessions = MentorshipSession.objects.filter(mentor__id=mentor.id,
                                                                           mentee__id=mentee.id,
                                                                           status__in=['PENDING', 'STARTED'])
        if unfinished_with_mentee_sessions.count() > 0:
            return unfinished_with_mentee_sessions

    # if I'm here its because there was no previous session
