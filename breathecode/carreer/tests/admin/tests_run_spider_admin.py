...
from breathecode.carreer.admin import run_spider_admin
...
class RunSpiderAdminTestSuite(CarreerTestCase):
    ...
    @patch('breathecode.carreer.actions.run_spider', MagicMock(side_effect=Exception('They killed kenny')))
    def test_run_spider_admin__with_zero_spider_logger_error(self):
        from breathecode.carreer.actions import run_spider
    ...
    @patch(DJANGO_CONTRIB_PATH['messages'], apply_django_contrib_messages_mock())
    @patch('breathecode.carreer.actions.run_spider', MagicMock())
