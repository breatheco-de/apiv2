...
from ..models import Platform, Spider, Job, Employer, Position, PositionAlias, CarreerTag, Location, LocationAlias, ZyteProject
from breathecode.utils import ValidationException
from breathecode.carreer.services.regex import _cases_salary, _cases_date
...
class BaseScraper(ABC):
    ...
    def save_tag(cls, keyword: str):
        ...
        tag = CarreerTag.objects.filter(slug__iexact=tag_slug).first()

        if tag is None:
            tag = CarreerTag(slug=tag_slug)
            tag.save()
