...
def event_by_url_param(context: ServiceContext, args: tuple, kwargs: dict) -> tuple[dict, tuple, dict]:
    ...
    if (
        context["price"] == 0
        and is_no_saas_student_up_to_date_in_any_cohort(context["request"].user, academy=event.academy) is False
    ):
    ...
    if event.ending_at < utc_now:
        raise ValidationException(
            translation(
                lang, en="This event has already finished", es="Este evento ya ha terminado", slug="event-has-ended"
            ),
            code=400,
        )
...
def live_class_by_url_param(context: ServiceContext, args: tuple, kwargs: dict) -> tuple[dict, tuple, dict]:
    ...
    live_class = LiveClass.objects.filter(
        cohort_time_slot__cohort__cohortuser__user=request.user, hash=kwargs.get("hash")
    ).first()

    if not live_class:
        raise ValidationException(
            translation(lang, en="Live class not found", es="Clase en vivo no encontrada", slug="not-found"), code=404
        )
    ...
    cohort_available_as_saas = (
        live_class.cohort_time_slot.cohort.available_as_saas is not None
        and live_class.cohort_time_slot.cohort.available_as_saas
    )

    # avoid to be taken if the cohort is available as saas is set
    academy_available_as_saas = (
        live_class.cohort_time_slot.cohort.available_as_saas is None
    ...
    if (
        context["price"] == 0
        and is_no_saas_student_up_to_date_in_any_cohort(
            context["request"].user, cohort=live_class.cohort_time_slot.cohort
        )
        is False
    ):
        raise PaymentException(
    ...
    if live_class.ending_at < utc_now:
        raise ValidationException(
            translation(lang, en="Class has ended", es="La clase ha terminado", slug="class-has-ended"), code=400
        )
