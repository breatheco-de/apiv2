...
class TestSyncOrgVenues(LegacyAPITestCase):
    ...
    def test_2_live_classes(self):
        ...
        self.bc.check.calls(
            tasks.fix_live_class_dates.delay.call_args_list,
            [
                call(1),
                call(2),
                call(3),
        ...
        self.bc.check.calls(
            ...
            [
                call("Found 2 cohorts that have not finished and should have live classes\n"),
                call(
                    f"Adding cohort {model.cohort[0].slug} to the fixing queue, it ends on "
                    f"{model.cohort[0].ending_date}\n"
                ),
                call(
                    f"Adding cohort {model.cohort[1].slug} to the fixing queue, it ends on "
