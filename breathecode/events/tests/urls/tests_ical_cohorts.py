...
class AcademyCohortTestSuite(EventTestCase):
    ...
    def test_ical_cohorts__with_one__with_incoming_true__return_zero_cohorts(self):
        ...
        device_id_kwargs = {"name": "server"}
        model = self.generate_models(
            academy=True, cohort=True, device_id=True, cohort_kwargs=cohort_kwargs, device_id_kwargs=device_id_kwargs
        )
    ...
    def test_ical_cohorts__with_one__first_day__last_day(self):
        ...
        last_timeslot_starting_at = datetime(
            year=2030, month=10, day=6, hour=0, minute=30, tzinfo=gettz("Europe/Madrid")
        )
        ...
        starting_at_fixed = self.datetime_to_ical(
            fix_datetime_weekday(model.cohort.kickoff_date, starting_at, next=True), utc=False
        )

        ending_at = DatetimeInteger.to_datetime(timeslot.timezone, timeslot.ending_at)

        ending_at_fixed = self.datetime_to_ical(
            fix_datetime_weekday(model.cohort.kickoff_date, ending_at, next=True), utc=False
        )
    ...
    def test_ical_cohorts__with_two__first_day__last_day__two_timeslots(self):
        ...
        model = self.generate_models(
            academy=True, cohort=(2, cohort_kwargs), device_id=device_id_kwargs, cohort_time_slot=cohort_time_slots
        )
        ...
        first_timeslot_starting_at = self.datetime_to_ical(
        ...
        first_timeslot_ending_at = self.datetime_to_ical(
            fix_datetime_weekday(
                model.cohort[0].kickoff_date,
        ...
        last_timeslot_starting_at = self.datetime_to_ical(
            fix_datetime_weekday(
        ...
        last_timeslot_ending_at = self.datetime_to_ical(
            fix_datetime_weekday(
    ...
    def test_ical_cohorts__with_two__first_day__last_day__two_timeslots__cohort_with_meeting_url(self):
        ...
        model = self.generate_models(
            academy=True, cohort=cohort_kwargs, device_id=device_id_kwargs, cohort_time_slot=cohort_time_slots
        )
