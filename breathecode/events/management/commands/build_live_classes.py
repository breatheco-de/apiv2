...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        cohorts = Cohort.objects.filter(ending_date__gte=utc_now, never_ends=False).exclude(stage__in=['DELETED', 'PREWORK'])
        ...
        for cohort in cohorts:
            ...
            if total_cohort_timeslots == 0:
                self.stdout.write(self.style.ERROR(f"Cohort {cohort.slug} live classes will not be generated because it does not have timeslots"))
            else:
                self.stdout.write(self.style.SUCCESS(f"Adding cohort {cohort.slug} live classes to the generation queue"))
