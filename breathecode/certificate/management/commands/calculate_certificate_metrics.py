...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if specialty_slug:
            ...
            if not specialties.exists():
                self.stdout.write(
                    self.style.ERROR(f"Specialty with slug '{specialty_slug}' not found")
                )
                return
        else:
        ...
        self.stdout.write(
            self.style.SUCCESS(f"Calculating metrics for {specialties.count()} specialties...")
        )
        ...
        for specialty in specialties:
            ...
            if dry_run:
                self.stdout.write(
                    f"[DRY RUN] Would update {specialty.slug}: {metrics}"
                )
            else:
                specialty.metrics = metrics
                specialty.save(update_fields=["metrics"])
                self.stdout.write(
                    self.style.SUCCESS(f"Updated {specialty.slug}: {metrics}")
                )
                updated_count += 1

        if dry_run:
            self.stdout.write(
                self.style.WARNING(f"[DRY RUN] Would have updated {specialties.count()} specialties")
            )
        else:
            self.stdout.write(
                self.style.SUCCESS(f"Successfully updated {updated_count} specialties")
            )
    ...
    def calculate_specialty_metrics(self, specialty):
        ...
        issued_certificates = UserSpecialty.objects.filter(
            specialty=specialty,
            status="PERSISTED"
