...
class AcademySpecialtiesView(APIView, GenerateLookupsMixin):
    ...
    def get(self, request, academy_id=None):
        ...
        items = (
            Specialty.objects.filter(syllabuses__academy_owner=academy_id).exclude(status=Specialty.DELETED).distinct()
        )
        ...
        items = items.extra(select={"total_issued": "COALESCE((metrics->>'total_issued')::integer, 0)"}).order_by(
            "-total_issued", "name"
        )
