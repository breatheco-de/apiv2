...
class ActionCertificateSetDefaultIssuedAtTestCase(CertificateTestCase):
    ...
    def test_issued_at_null_status_error(self):
        ...
        model = self.generate_models(
            user_specialty=True, cohort=False, user_specialty_kwargs={"status": "ERROR", "issued_at": None}
        )
    ...
    def test_issued_at_set_status_error(self):
        ...
        model = self.generate_models(
            user_specialty=True, cohort=False, user_specialty_kwargs={"status": "ERROR", "issued_at": now}
        )
    ...
    def test_issued_at_null_status_persisted_one_item(self):
        ...
        model = self.generate_models(
            user_specialty=True, cohort=False, user_specialty_kwargs={"status": "PERSISTED", "issued_at": None}
        )
    ...
    def test_issued_at_null_status_persisted_two_items(self):
        ...
        model1 = self.generate_models(
            user_specialty=True,
            cohort=False,
            user_specialty_kwargs={"status": "PERSISTED", "issued_at": None, "token": "123abcd"},
        )
        model2 = self.generate_models(
            user_specialty=True,
            cohort=False,
            user_specialty_kwargs={"status": "PERSISTED", "issued_at": None, "token": "567pqrst"},
        )
    ...
    def test_issued_at_null_status_persisted_one_item_with_cohort(self):
        ...
        model = self.generate_models(
            user_specialty=True, cohort=True, user_specialty_kwargs={"status": "PERSISTED", "issued_at": None}
        )
    ...
    def test_issued_at_set_status_persisted(self):
        ...
        now = timezone.now()
        model = self.generate_models(
            user_specialty=True, user_specialty_kwargs={"status": "PERSISTED", "issued_at": now}
        )
        ...
        self.assertEqual(
            str(signals.user_specialty_saved.send_robust.call_args_list),
            str(
                [
                    call(instance=model.user_specialty, sender=model.user_specialty.__class__),
                ]
    ...
    def test_issued_at_null_status_pending(self):
        ...
        model = self.generate_models(
            user_specialty=True, user_specialty_kwargs={"status": "PENDING", "issued_at": None}
        )
