...
class CertificateTestSuite(CertificateTestCase):
    ...
    def test_certificate_re_attemps_two_certificates(self):
        ...
        base = self.generate_models(authenticate=True, cohort=True, cohort_finished=True,
            capability='crud_certificate', role='STUDENT', profile_academy=True, 
            syllabus=True, specialty=True, layout_design=True, cohort_stage="ENDED")
        
        del base['user']

        user_specialty_1_kwargs = {
            "token": "qwerrty"
        }
        user_specialty_2_kwargs = {
            "token": "huhuhuhuhu"
        }

        models = [
            self.generate_models(user=True, cohort_user=True,
                cohort_user_educational_status='GRADUATED', 
                cohort_user_finantial_status='UP_TO_DATE',user_specialty=True, 
                user_specialty_kwargs=user_specialty_2_kwargs, models=base),
            self.generate_models(user=True, cohort_user=True,
                cohort_user_educational_status='GRADUATED', 
                cohort_user_finantial_status='UP_TO_DATE',user_specialty=True, 
                user_specialty_kwargs=user_specialty_1_kwargs, models=base),
        ]
        ...
        expected = [
            {
                ...
                'cohort': {
                    ...
                    'syllabus': {
                        'certificate': {
                            'duration_in_hours': models[0].certificate.duration_in_hours
                        }
                    }
                },
            ...
            },
            {
                ...
                'cohort': {
                    ...
                    'syllabus': {
                        'certificate': {
                            'duration_in_hours': models[1].certificate.duration_in_hours
                        }
                    }
                },
