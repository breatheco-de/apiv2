    ...
    patch_get([
        {
            'method': 'GET',
            'url':
            f'https://api.github.com/orgs/{model.academy_auth_settings.github_username}/repos?page=1&type=forks&per_page=30&sort=created&direction=desc',
            'expected': [],
            'code': 200,
            'headers': {}
        },
...
def test_two_repos__deleting_repositories(database: capyc.Database, patch_get, set_datetime, utc_now):
    ...
    model = database.create(academy_auth_settings=1,
                            city=1,
                            country=1,
                            user=1,
    ...
    assert database.list_of('assignments.RepositoryDeletionOrder') == [
        {
            ...
            'status': 'ERROR',
            'status_text':
            'Unable to communicate with Github API for /repos/breatheco-de/curso-nodejs-4geeks, error: 404',
        },
        {
            'id': 2,
            'provider': 'GITHUB',
            ...
            'status': 'ERROR',
            'status_text':
            'Unable to communicate with Github API for /repos/4GeeksAcademy/curso-nodejs-4geeks, error: 404',
        },
    ]
    assert database.list_of('assignments.RepositoryWhiteList') == []
...
@pytest.mark.parametrize('attr, is_readme, is_joined', [
    ('url', False, False),
...
def test_two_repos_scheduled_and_in_this_execution_was_added_to_the_assets(database: capyc.Database, patch_get, attr,
                                                                           is_readme, is_joined):
    if is_readme and is_joined:
        assets = [
            {
