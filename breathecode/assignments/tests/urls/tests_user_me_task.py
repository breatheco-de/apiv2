...
class MediaTestSuite(AssignmentsTestCase):
    ...
    def test_user_me_task__with_query_string_assets(self):
        model = self.bc.database.create(
            user=1, task=[{"associated_slug": "fine"}, {"associated_slug": "super"}], cohort=1
        )
        self.client.force_authenticate(model.user)
            ...
            "failure": [
                {
                    ...
                    "resources": [
                        {"display_field": "pk", "display_value": 1, "pk": 1},
                        {"display_field": "pk", "display_value": 2, "pk": 2},
                    ],
                    "status_code": 404,
                }
            ],
            ...
            "failure": [
                {
                    ...
                    "resources": [
                        {"display_field": "associated_slug", "display_value": x.associated_slug, "pk": x.pk}
                        for x in model.task
                    ],
                    "status_code": 400,
                }
    ...
    def test_put_passing_random_values_to_update_task(self):
        ...
        self.assertEqual(
            ...
            [{**self.bc.format.to_dict(model.task[x]), **data[x]} for x in range(0, 2)],
        )
        self.bc.check.calls(
            ...
            [
                ...
                for x in model.task
                if data[x.id - 1]["task_status"] != x.task_status
            ],
        )
...
def test_post__created(
    client: capy.Client, database: capy.Database, fake: capy.Fake, task_type: str, utc_now: datetime
):
    url = reverse_lazy("assignments:user_me_task")
...
def test_post__marks_exercise_done_when_learnpack_telemetry_is_complete(client: capy.Client, database: capy.Database):
    ...
    assert response.status_code == status.HTTP_201_CREATED
    task = Task.objects.get(associated_slug="speaking-with-ai", cohort=model.cohort)
    assert task.task_status == "DONE"
    assert task.revision_status == "APPROVED"
    assert task.telemetry_id == telemetry.id
    assert response.json()[0]["task_status"] == "DONE"
