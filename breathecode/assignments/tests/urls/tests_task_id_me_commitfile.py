...
class MediaTestSuite(AssignmentsTestCase):
    ...
    def test__no_tasks(self):
        ...
        url = (
            reverse_lazy("assignments:me_task_id_commitfile", kwargs={"task_id": 1})
            + "?"
    ...
    def test__no_github_Credentials(self):
        ...
        self.assertEqual(
            response.getvalue().decode("utf-8"), '{"detail":"github-account-not-connected","status_code":400}'
        )
        ...
        with patch("linked_services.django.actions.get_jwt", MagicMock(return_value=token)):
            with patch.multiple("requests", get=MagicMock(return_value=mock)):
                ...
                self.bc.check.calls(
                    ...
                    [
                        call(
                            model.app.app_url + "/v1/finetuning/commitfile",
                            params={
                                **query,
