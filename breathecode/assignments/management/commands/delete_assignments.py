        ...
        for a in Task.objects.all().reverse():
            ...
            if b is not None:
                if a.task_status == 'PENDING':
                    a.delete()
                    self.stdout.write(self.style.SUCCESS(f"Student: {a.user.email} task {a.associated_slug} with status {a.task_status} was deleted"))
                elif b.task_status == 'PENDING':
                    b.delete()
                    self.stdout.write(self.style.SUCCESS(f"Student: {b.user.email} task {b.associated_slug} with status {b.task_status} was deleted"))
                else:
                    a.delete()
                    self.stdout.write(self.style.SUCCESS(f"Student: {a.user.email} task {a.associated_slug} with status {a.task_status} was deleted"))
