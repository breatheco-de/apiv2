...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            ...
            type=int,
            default=50,
            help='Number of repositories to process in each batch (default: 50)'
        )
        parser.add_argument(
            '--dry-run',
            action='store_true',
        ...
        parser.add_argument(
            ...
            type=int,
            default=2023,
            help='Year to filter cohorts by ending date (default: 2023)'
        )
        parser.add_argument(
            '--max-deletions',
            type=int,
            default=100,
            help='Maximum number of repositories to delete (default: 100)'
        )
    ...
    def handle(self, *args, **options):
        ...
        if self.dry_run:
            self.stdout.write(
                self.style.WARNING(f"DRY RUN MODE: No repositories will be deleted")
            )
        ...
        self.stdout.write(
            self.style.SUCCESS(f"Found {eligible_cohorts.count()} eligible cohorts")
        )
        
        # Get tasks with GitHub URLs from eligible cohorts
        tasks_with_repos = self.get_tasks_with_repositories(eligible_cohorts)
        self.stdout.write(
            self.style.SUCCESS(f"Found {len(tasks_with_repos)} tasks with GitHub repositories")
        )
    ...
    def get_eligible_cohorts(self) -> 'QuerySet[Cohort]':
        ...
        return Cohort.objects.filter(
            stage='ENDED',
            ending_date__gte=year_start,
            ending_date__lt=year_end,
            never_ends=False
    ...
    def get_tasks_with_repositories(self, cohorts: 'QuerySet[Cohort]') -> list[Task]:
        ...
        return list(
            ...
            ).exclude(github_url='').select_related('cohort', 'user')
        )
    ...
    def process_organization_repositories(self, org_name: str, tasks: list[Task]) -> int:
        ...
        self.stdout.write(
            self.style.SUCCESS(f"Processing {len(tasks)} repositories for organization: {org_name}")
        )
        ...
        if not github_client:
            self.stdout.write(
                self.style.ERROR(f"No GitHub credentials found for organization: {org_name}")
            )
            return 0
        ...
        self.stdout.write(
            self.style.SUCCESS(f"Found {len(repositories)} unique repositories to process")
        )
        ...
        for i in range(0, len(repositories), self.batch_size):
            ...
            if self.total_deleted + deleted_count >= self.max_deletions:
                ...
                if remaining_limit <= 0:
                    self.stdout.write(
                        self.style.WARNING(
                            f"Reached maximum deletion limit. Stopping batch processing."
                        )
                    )
                    break
                # Process only the remaining repositories up to the limit
                batch = repositories[i:i + min(self.batch_size, remaining_limit)]
            else:
            ...
            if self.total_deleted + deleted_count >= self.max_deletions:
                self.stdout.write(
                    self.style.WARNING(
                        f"Reached maximum deletion limit of {self.max_deletions} repositories."
                    )
                )
                break
    ...
    def delete_repository(self, github_client: Github, owner: str, repo_name: str) -> bool:
        ...
        if self.dry_run:
            self.stdout.write(
                self.style.WARNING(f"[DRY RUN] Would delete repository: {owner}/{repo_name}")
            )
            return True
        ...
        try:
            ...
            if response.status_code == 204:  # GitHub returns 204 for successful deletion
                self.stdout.write(
                    self.style.SUCCESS(f"Successfully deleted repository: {owner}/{repo_name}")
                )
            ...
            self.stdout.write(
                self.style.ERROR(f"Error deleting repository {owner}/{repo_name}: {str(e)}")
            )
