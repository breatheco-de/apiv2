...
class Command(BaseCommand):
    ...
    def github(self):
        ...
        for settings in AcademyAuthSettings.objects.filter(
                github_owner__isnull=False, github_owner__credentialsgithub__isnull=False).exclude(github_username=''):
            self.github_client = Github(org=settings.github_username,
            ...
            last = RepositoryDeletionOrder.objects.filter(
                provider=RepositoryDeletionOrder.Provider.GITHUB).only('created_at').last()
            if last:
    ...
    def purge_deletion_orders(self):
        ...
        while True:
            qs = RepositoryDeletionOrder.objects.filter(
                status=RepositoryDeletionOrder.Status.PENDING, )[page * 100:(page + 1) * 100]

            if len(qs) == 0:
                break

            for deletion_order in qs:
                if RepositoryWhiteList.objects.filter(provider=deletion_order.provider,
                                                      repository_user__iexact=deletion_order.repository_user,
                                                      repository_name__iexact=deletion_order.repository_name).exists():
    ...
    def delete_github_repositories(self):
        ...
        while True:
            qs = RepositoryDeletionOrder.objects.filter(provider=RepositoryDeletionOrder.Provider.GITHUB,
                                                        status=RepositoryDeletionOrder.Status.PENDING,
                                                        created_at__lte=timezone.now() - relativedelta(months=2))[:100]
            ...
            for deletion_order in qs:
                try:
                    self.github_client.delete_org_repo(owner=deletion_order.repository_user,
                                                       repo=deletion_order.repository_name)
    ...
    def fill_whitelist(self):
        ...
        for asset in assets:
            options = [
                asset.url, asset.solution_url, asset.preview, asset.readme_url, asset.intro_video_url,
    ...
    def add_to_whitelist(self, provider: str, user: str, repo_name: str):
        if RepositoryWhiteList.objects.filter(provider=provider,
                                              repository_user__iexact=user,
                                              repository_name__iexact=repo_name).exists() is False:
            RepositoryWhiteList.objects.get_or_create(provider=provider,
                                                      repository_user=user,
                                                      repository_name=repo_name)

    def schedule_github_deletions(self, organization: str, last_check: Optional[datetime] = None):
        for repos in self.github_client.get_org_repos(organization,
                                                      type='forks',
                                                      per_page=30,
                                                      direction='desc',
                                                      sort='created'):
    ...
    def schedule_github_deletion(self, provider: str, user: str, repo_name: str):
        if RepositoryWhiteList.objects.filter(provider=provider, repository_user=user,
                                              repository_name=repo_name).exists():
            return

        RepositoryDeletionOrder.objects.get_or_create(provider=provider,
                                                      repository_user=user,
                                                      repository_name=repo_name)
