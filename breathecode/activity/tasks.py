...
def upload_activities(self, task_manager_id: int, **_):
    ...
    def extract_data():
        ...
        while True:
            try:
                with Lock(client, f'lock:activity:worker-{worker}', timeout=0.4, blocking_timeout=0.4):
...
def add_activity(user_id: int,
    ...
    try:
        with Lock(client, 'lock:activity:current-worker', timeout=0.4, blocking_timeout=0.4):
