...
class AcademyCohortTestSuite(AdmissionsTestCase):
    ...
    def test_academy_cohort__without_capability(self):
        ...
        self.assertEqual(
            json,
            {"detail": "You (user: 1) don't have this capability: read_all_cohort for academy 1", "status_code": 403},
        )
    ...
    def test_academy_cohort__post__without_timezone(self):
        ...
        self.assertEqual(self.all_cohort_time_slot_dict(), [])
        self.assertEqual(
            cohort_saved.send_robust.call_args_list, [call(instance=cohort, sender=cohort.__class__, created=True)]
        )
    ...
    def test_academy_cohort__with_data__with_sort(self):
        ...
        base = self.bc.database.create(
            authenticate=True, profile_academy=True, capability="read_all_cohort", role="potato", skip_cohort=True
        )

        models = [
            self.bc.database.create(
                cohort=cohort_kwargs, syllabus=True, syllabus_version=True, syllabus_schedule=True, models=base
            )
            for _ in range(0, 2)
        ]
    ...
    def test_academy_cohort_delete_without_args_in_url_or_bulk(self):
        ...
        self.headers(academy=1)
        model = self.bc.database.create(
            authenticate=True, profile_academy=True, capability="crud_cohort", role="potato"
        )
    ...
    def test_academy_cohort_delete_in_bulk_with_two(self):
        ...
        for field in many_fields:
            ...
            model1 = self.bc.database.create(
                authenticate=True, profile_academy=True, syllabus=True, cohort=cohort_kwargs, models=base
            )
