...
class CertificateTestSuite(AdmissionsTestCase):
    ...
    def test_academy_cohort_sync_timeslot__with_cohort_in_querystring__with_certificate(self):
        ...
        url = reverse_lazy('admissions:academy_cohort_sync_timeslot') + '?cohort=1'
        model = self.generate_models(authenticate=True, profile_academy=True,
            capability='crud_certificate', role='potato', certificate=True,
            syllabus=True)
    ...
    def test_academy_cohort_sync_timeslot__with_one_certificate_timeslot(self):
        ...
        model = self.generate_models(authenticate=True, profile_academy=True,
            capability='crud_certificate', role='potato', certificate=True,
            syllabus=True, certificate_time_slot=True)
    ...
    def test_academy_cohort_sync_timeslot__with_two_certificate_timeslot(self):
        ...
        models = [self.generate_models(certificate_time_slot=True, models=base)
            for _ in range(0, 2)]
    ...
    def test_academy_cohort_sync_timeslot__with_two_certificate_timeslot__with_two_cohort(self):
        ...
        base = self.generate_models(authenticate=True, profile_academy=True,
            capability='crud_certificate', role='potato', certificate=True,
            syllabus=True, skip_cohort=True)

        cohorts = [self.generate_models(cohort=True, models=base).cohort
            for _ in range(0, 2)]

        certificate_timeslots = [self.generate_models(certificate_time_slot=True,
            models=base).certificate_time_slot for _ in range(0, 2)]
        ...
        expected = [{
            ...
            'cohort': 1,
            'starting_at': self.datetime_to_iso(certificate_time_slot.starting_at),
            'ending_at': self.datetime_to_iso(certificate_time_slot.ending_at),
    ...
    def test_academy_cohort_sync_timeslot__with_one_cohort_timeslot(self):
        ...
        model = self.generate_models(authenticate=True, profile_academy=True,
            capability='crud_certificate', role='potato', certificate=True,
            syllabus=True, cohort_time_slot=True, certificate_time_slot=True)
