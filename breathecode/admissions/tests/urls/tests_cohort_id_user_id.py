...
class CohortIdUserIdTestSuite(AdmissionsTestCase):
    ...
    def test_cohort_id_user_id_put_with_bad_id(self):
        ...
        url = reverse_lazy(
            "admissions:cohort_id_user_id", kwargs={"cohort_id": model.cohort.id, "user_id": model.user.id}
        )
        data = {}
        response = self.client.put(url, data, format="json")
        json = response.json()
    ...
    def test_cohort_id_user_id_put_with_id(self):
        ...
        model = self.generate_models(
            authenticate=True, cohort=True, user=True, syllabus_schedule=True, profile_academy=True, cohort_user=True
        )
