...
class CohortIdUserIdTestSuite(AdmissionsTestCase):
    ...
    def test__post__have_a_subscription_or_a_plan_financing(self):
        ...
        model = self.bc.database.create(
            user=1, cohort=cohort, cohort_set=1, cohort_set_cohort=1, academy=academy, **extra
        )
        self.client.force_authenticate(model.user)
    ...
    def test__post__joined_to_cohort(self):
        ...
        academy = {"available_as_saas": True}
        model = self.bc.database.create(
            user=1, cohort=cohort, cohort_set=1, cohort_set_cohort=1, cohort_user=1, academy=academy, **extra
        )
        self.client.force_authenticate(model.user)
    ...
    def test__post__joined_to_another_endable_cohort(self):
        ...
        academy = {"available_as_saas": True}
        model = self.bc.database.create(
            user=1, cohort=(2, cohort), cohort_set=1, cohort_set_cohort=1, academy=academy, **extra
        )
        self.client.force_authenticate(model.user)
    ...
    def test__post__joined_to_another_endable_cohort(self):
        ...
        for cohort1, cohort2 in cohorts:
            ...
            expected = post_serializer(
                self, resource, model.cohort_set, [model.cohort[0], model.cohort[1]], model.academy, model.user, data={}
            )
