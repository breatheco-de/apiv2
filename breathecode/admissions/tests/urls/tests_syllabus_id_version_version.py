...
class CertificateTestSuite(AdmissionsTestCase):
    ...
    def test_syllabus_id_version_version_without_auth(self):
        """Test /certificate without auth"""
        self.headers(academy=1)
        url = reverse_lazy("admissions:syllabus_id_version_version", kwargs={"syllabus_id": 1, "version": 1})
    ...
    def test_syllabus_id_version_version_without_capability(self):
        """Test /certificate without auth"""
        self.headers(academy=1)
        url = reverse_lazy("admissions:syllabus_id_version_version", kwargs={"syllabus_id": 1, "version": 1})
    ...
    def test_syllabus_id_version_version_without_syllabus(self):
        """Test /certificate without auth"""
        self.headers(academy=1)
    ...
    def test_syllabus_id_version_version(self):
        """Test /certificate without auth"""
        self.headers(academy=1)
        model = self.generate_models(
            authenticate=True,
        ...
        url = reverse_lazy(
            "admissions:syllabus_id_version_version",
            kwargs={"syllabus_id": 1, "version": model.syllabus_version.version},
        )
        response = self.client.get(url)
        json = response.json()
        expected = {
