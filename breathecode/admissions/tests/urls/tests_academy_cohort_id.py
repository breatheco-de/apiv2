...
class AcademyCohortIdTestSuite(AdmissionsTestCase):
    ...
    def test_cohort_id_put__without_capability(self):
        ...
        self.assertEqual(
            json, {"detail": "You (user: 1) don't have this capability: crud_cohort for academy 1", "status_code": 403}
        )
    ...
    def test_cohort_id__put__without_cohort(self):
        ...
        model = self.generate_models(
            authenticate=True, profile_academy=True, capability="crud_cohort", role="potato", syllabus=True
        )
    ...
    def test_cohort_id__put__kickoff_date_greater_than_ending_date(self):
        ...
        model = self.generate_models(
            authenticate=True, cohort=cohort, profile_academy=True, capability="crud_cohort", role="potato"
        )
    ...
    def test_cohort_id__put__with_id__with_bad_syllabus_version(self):
        ...
        self.headers(academy=1)
        model = self.generate_models(
            authenticate=True, cohort=True, profile_academy=True, capability="crud_cohort", role="potato", syllabus=True
        )
            ...
            "timeslots": [
                {
                    "ending_at": DatetimeInteger.to_iso_string(
                        model.academy.timezone, syllabus_schedule_time_slot.ending_at
                    ),
                    ...
                    "starting_at": DatetimeInteger.to_iso_string(
                        model.academy.timezone, syllabus_schedule_time_slot.starting_at
                    ),
                }
    ...
    def test_cohort_id__put__with_id__with_data_in_body__cohort_with_timezone(self):
        ...
        expected = {
            ...
            "timeslots": [
                {
                    "ending_at": DatetimeInteger.to_iso_string(
                        model.cohort.timezone, syllabus_schedule_time_slot.ending_at
                    ),
                    "id": syllabus_schedule_time_slot.id,
                    "recurrency_type": syllabus_schedule_time_slot.recurrency_type,
                    "recurrent": syllabus_schedule_time_slot.recurrent,
                    "starting_at": DatetimeInteger.to_iso_string(
                        model.cohort.timezone, syllabus_schedule_time_slot.starting_at
                    ),
                }
                for syllabus_schedule_time_slot in model2.syllabus_schedule_time_slot
            ],
    ...
    def test_academy_cohort_id__delete__cohort_with_students(self):
        ...
        self.headers(academy=1)
        model = self.generate_models(
            authenticate=True, profile_academy=True, capability="crud_cohort", role="potato", cohort_user=True
        )
