...
class CohortUserTestSuite(AdmissionsTestCase):
    ...
    def test_put_in_bulk_with_stage_delete(self):
        ...
        url = reverse_lazy("admissions:cohort_user")
        model = self.generate_models(
            authenticate=True, cohort_user=True, profile_academy=True, cohort_kwargs=cohort_kwargs
        )
        data = [{"id": model["cohort_user"].id}]
        response = self.client.put(url, data, format="json")
        json = response.json()
    ...
    def test_cohort_id_user_delete_in_bulk_with_one(self):
        ...
        for field in many_fields:
            ...
            url = reverse_lazy("admissions:cohort_user") + f"?{field}=" + str(getattr(model["cohort_user"], field))
            response = self.client.delete(url)
    ...
    def test_cohort_id_user_delete_in_bulk_with_two(self):
        ...
        for field in many_fields:
            ...
            url = (
                reverse_lazy("admissions:cohort_user")
                + f"?{field}="
