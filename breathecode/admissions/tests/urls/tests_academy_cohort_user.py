...
class CohortUserTestSuite(AdmissionsTestCase):
    ...
    def test__with_data(self):
        ...
        self.headers(academy=1)
        model = self.generate_models(
            authenticate=True, cohort_user=True, profile_academy=True, capability="read_all_cohort", role="potato"
        )
    ...
    @patch("django.db.models.signals.pre_delete.send_robust", MagicMock(return_value=None))
    @patch("breathecode.admissions.signals.student_edu_status_updated.send_robust", MagicMock(return_value=None))
    def test__with_data__with_cohorts_get_plans_with_no_plans(self):
        """Test /cohort/user without auth"""
        self.headers(academy=1)
        model = self.generate_models(
            authenticate=True,
        ...
        model_dict = self.remove_dinamics_fields(model["cohort_user"].__dict__)
        base_url = reverse_lazy("admissions:academy_cohort_user")
        url = f"{base_url}?cohorts=" + model["cohort_user"].cohort.slug + "&plans=True"
        response = self.client.get(url)
        json = response.json()
        expected = [
            {
    ...
    @patch("django.db.models.signals.pre_delete.send_robust", MagicMock(return_value=None))
    @patch("breathecode.admissions.signals.student_edu_status_updated.send_robust", MagicMock(return_value=None))
    def test__with_data__with_cohorts_get_plans_with_plans(self):
        """Test /cohort/user without auth"""
        self.headers(academy=1)
        model = self.generate_models(
            authenticate=True,
    ...
    @patch("django.db.models.signals.pre_delete.send_robust", MagicMock(return_value=None))
    @patch("breathecode.admissions.signals.student_edu_status_updated.send_robust", MagicMock(return_value=None))
    def test__with_data__with_cohorts_get_plans_financing(self):
        """Test /cohort/user without auth"""
        self.headers(academy=1)
        model = self.generate_models(
            authenticate=True,
                ...
                "tasks": [
                    {
                        ...
                        "delivered_at": (
                            ...
                            if model["task"].delivered_at is not None
                            else None
                        ),
    ...
    def test__put__in_bulk__with_two_items(self):
        ...
        data = [
            ...
            {"user": "2", "cohort": "2", "educational_status": "GRADUATED"},
        ]
        response = self.client.put(url, data, format="json")
        json = response.json()
    ...
    def test__post__in_bulk__0_items(self):
        ...
        self.headers(academy=1)
        model = self.generate_models(
            authenticate=True, cohort=True, profile_academy=True, capability="crud_cohort", role="potato"
        )
        url = reverse_lazy("admissions:academy_cohort_user")
        data = []
    ...
    def test__delete__in_bulk__with_one(self):
        ...
        base = self.generate_models(authenticate=True, profile_academy=True, capability="crud_cohort", role="potato")
        ...
        for field in many_fields:
            ...
            url = (
                reverse_lazy("admissions:academy_cohort_user")
                + f"?{field}="
    ...
    def test__delete__in_bulk__with_two(self):
        ...
        for field in many_fields:
            ...
            model1 = self.generate_models(cohort_user=True, cohort_user_kwargs=cohort_user_kwargs, models=base)
            ...
            model2 = self.generate_models(cohort_user=True, cohort_user_kwargs=cohort_user_kwargs, models=base)
            url = (
                reverse_lazy("admissions:academy_cohort_user")
                + f"?{field}="
