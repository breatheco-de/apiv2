...
class CohortUserTestSuite(MarketingTestCase):
    ...
    @patch(GOOGLE_CLOUD_PATH["bucket"], apply_google_cloud_bucket_mock())
    @patch(GOOGLE_CLOUD_PATH["blob"], apply_google_cloud_blob_mock())
    def test_lead_all__with_bad_academy_id_in_querystring(self):
        """Test /lead/all with invalid academy id in querystring"""
        self.headers(academy=1)
        url = reverse_lazy("marketing:lead_all") + "?academy=999999"
        model = self.generate_models(
            authenticate=True, profile_academy=True, capability="read_lead", role="potato", form_entry=True
        )
    ...
    @patch(GOOGLE_CLOUD_PATH["bucket"], apply_google_cloud_bucket_mock())
    @patch(GOOGLE_CLOUD_PATH["blob"], apply_google_cloud_blob_mock())
    def test_lead_all__with_academy_id_in_querystring(self):
        """Test /lead/all with academy id in querystring"""
        self.headers(academy=1)
        model = self.generate_models(
            authenticate=True,
            profile_academy=True,
        ...
        url = reverse_lazy("marketing:lead_all") + f"?academy={model.academy.id}"
        response = self.client.get(url)
        json = response.json()
    ...
    @patch(GOOGLE_CLOUD_PATH["bucket"], apply_google_cloud_bucket_mock())
    @patch(GOOGLE_CLOUD_PATH["blob"], apply_google_cloud_blob_mock())
    def test_lead_all__with_two_academy_id_in_querystring(self):
        """Test /lead/all with two academy ids in querystring"""
        self.headers(academy=1)
        base = self.generate_models(user=True)
        ...
        models.sort(key=lambda x: x.form_entry.created_at)
        url = reverse_lazy("marketing:lead_all") + "?academy=" + ",".join([str(x.academy.id) for x in models])
        response = self.client.get(url)
        json = response.json()
