...
from breathecode.websocket.decorators import ws_capable_of
...
capability1 = fake.slug()
capability2 = fake.slug()


@ws_capable_of(capability1)
...
class ConsumerTestSuite(WebsocketTestCase):
    ...
    async def test__async__with_token(self):
        ...
        self.assertEqual(
            message, {
                'details': "Missing academy_id parameter expected for the endpoint url or 'Academy' header",
                'status_code': 403
            })
    ...
    async def test__async__with_capability(self):
        model = await self.bc.database.async_create(token=1,
                                                    user=1,
                                                    profile_academy=1,
