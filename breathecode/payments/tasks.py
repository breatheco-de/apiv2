...
@task(bind=False, priority=TaskPriority.BACKGROUND.value)
def calculate_recent_coupon_stats(academy_id: Optional[int] = None, **kwargs: Any):
    """
    ...
    Args:
        academy_id: Optional academy ID. If None, processes all academies and global plans.
    """
    from datetime import timedelta
    
    logger.info(f"Starting calculate_recent_coupon_stats for academy_id={academy_id}")
    ...
    if academy_id is None:
        logger.info("Processing global-only coupons (owner=None) with default config")
        try:
            ...
            if count > 0:
                ...
                for coupon in global_coupons:
                    try:
                        ...
                        total_processed += 1
                        logger.debug(f"Updated stats for global coupon {coupon.slug} (used {coupon.times_used} times)")
                    except Exception as e:
                        total_errors += 1
                        logger.error(f"Error calculating stats for global coupon {coupon.slug}: {e}", exc_info=True)
                        continue
                
                logger.info(f"Global plans: Successfully processed {count} coupons")
        except Exception as e:
            total_errors += 1
            logger.error(f"Error processing global plans: {e}", exc_info=True)
        ...
        try:
            ...
            for coupon in academy_coupons:
                try:
                    ...
                    coupon.save(update_fields=["stats", "stats_updated_at"])
                    total_processed += 1
                    logger.debug(f"Updated stats for coupon {coupon.slug} (used {coupon.times_used} times)")
                except Exception as e:
                    total_errors += 1
                    logger.error(f"Error calculating stats for coupon {coupon.slug}: {e}", exc_info=True)
                    continue
            
            logger.info(f"Academy {academy.id}: Successfully processed {count} coupons")
            
        except Exception as e:
            total_errors += 1
            logger.error(f"Error processing academy {academy.id}: {e}", exc_info=True)
            continue
    
    logger.info(f"calculate_recent_coupon_stats completed: {total_processed} processed, {total_errors} errors")


@task(bind=False, priority=TaskPriority.BACKGROUND.value)
def cleanup_stale_coupon_stats(academy_id: Optional[int] = None, **kwargs: Any):
    """
    ...
    logger.info(f"Starting cleanup_stale_coupon_stats for academy_id={academy_id}")
    
    total_cleaned = 0
    
    # Process global plans once (using default config)
    if academy_id is None:
        logger.info("Cleaning up stale stats for global plans (owner=None) with default config")
        try:
            default_config = actions.get_coupon_stats_config(None)
            coupons_config = default_config.get("coupons", {})
            ...
            if count > 0:
                ...
                logger.debug("Global plans: No stale coupon stats to clean up")
        except Exception as e:
            logger.error(f"Error cleaning up stats for global plans: {e}", exc_info=True)
    ...
    for academy in academies:
        try:
            ...
            if count > 0:
                ...
                logger.debug(f"Academy {academy.id}: No stale coupon stats to clean up")
                
        except Exception as e:
            logger.error(f"Error cleaning up stats for academy {academy.id}: {e}", exc_info=True)
            continue
    
    logger.info(f"cleanup_stale_coupon_stats completed: {total_cleaned} coupons cleaned")
