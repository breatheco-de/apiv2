...
def patch_get_user(monkeypatch: pytest.MonkeyPatch):
    ...
    def patch(user):
        monkeypatch.setattr("breathecode.payments.views.AppConsumeView.get_user", MagicMock(return_value=user))
...
def endpoint_mock(monkeypatch: pytest.MonkeyPatch):
    ...
    monkeypatch.setattr(
        "breathecode.payments.views.ConsumeView.put",
...
def test_no_auth(bc: Breathecode, client: capy.Client):
    url = reverse_lazy("payments:app_service_slug_consumptionsession", kwargs={"service_slug": "slug"})
