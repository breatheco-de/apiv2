...
def patch_get_user(monkeypatch: pytest.MonkeyPatch):
    ...
    def patch(user):
        monkeypatch.setattr(
            "breathecode.payments.views.AppCancelConsumptionView.get_user", MagicMock(return_value=user)
        )
...
def endpoint_mock(monkeypatch: pytest.MonkeyPatch):
    ...
    monkeypatch.setattr(
        "breathecode.payments.views.CancelConsumptionView.put",
...
def test_no_auth(bc: Breathecode, client: capy.Client):
    url = reverse_lazy(
        "payments:app_service_slug_consumptionsession_id", kwargs={"service_slug": "slug", "consumptionsession_id": 1}
    )
...
def test_user_not_provided(
    ...
    url = reverse_lazy(
        "payments:app_service_slug_consumptionsession_id",
        kwargs={"service_slug": "slug", "consumptionsession_id": 1},
    )
    response = client.put(url)
