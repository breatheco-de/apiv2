...
class Command(BaseCommand):
    ...
    def subscriptions(self):
        ...
        no_need_to_renew = ServiceStockScheduler.objects.filter(
            consumables__valid_until__gte=self.utc_now +
            timedelta(hours=2)).exclude(plan_handler__subscription__status='CANCELLED').exclude(
                plan_handler__subscription__status='DEPRECATED').exclude(
                    plan_handler__subscription__status='PAYMENT_ISSUE')
    ...
    def plan_financing(self):
        plan_financings = PlanFinancing.objects.filter().exclude(status__in=['CANCELLED', 'DEPRECATED']).only(
            'id', 'user')
        ...
        no_need_to_renew = ServiceStockScheduler.objects.filter(
            consumables__valid_until__gte=self.utc_now + timedelta(hours=2)).exclude(
