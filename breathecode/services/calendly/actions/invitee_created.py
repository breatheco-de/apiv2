...
def invitee_created(self, webhook, payload: dict):
    ...
    service = None
    service_slug = payload['tracking']['utm_term']
    if service_slug is None:
        raise Exception(f'Missing service information on calendly iframe info: tracking->utm_term')
    ...
    if mentee is None:
        raise Exception(f'Mentee user not found with email {mentee_email}')
