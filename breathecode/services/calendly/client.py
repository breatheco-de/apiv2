...
class Calendly:
    ...
    def subscribe(self, org_username, org_hash):
    ...
    def unsubscribe_all(self, org_username):
        query_string = {'expand': 'organizer', 'status': 'live'}
        data = self.get_subscriptions(org_username)
        for webhook in data:
            self.unsubscribe(urlparse(webhook['uri']).path.split('/')[-1])

    def get_subscriptions(self, org_username):
