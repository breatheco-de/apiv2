...
def deal_add(self, webhook, payload: dict):
    ...
    if entry is None and 'deal[contactid]' in payload:
        entry = FormEntry.objects.filter(ac_contact_id=payload['deal[contactid]'], ac_deal_id__isnull=True).order_by('created_at').first()
    if entry is None and 'deal[contact_email]' in payload:
        entry = FormEntry.objects.filter(email=payload['deal[contact_email]'], ac_deal_id__isnull=True).order_by('created_at').first()
    if entry is None:
    ...
    logger.debug("Form Entry successfuly updated with deal information")
    return True
