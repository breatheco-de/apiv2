...
from breathecode.services.slack import actions, commands
...
class Slack:
    ...
    def _call(self, method_name, action_name, params=None, json=None):
        ...
        resp = requests.request(
            method=method_name, url=self.HOST + action_name, headers=self.headers, params=params, json=json, timeout=10
        )
    ...
    def execute_command(self, context):
        ...
        if hasattr(commands, _commands[0]):
            ...
            raise SlackException(
                f"No implementation has been found for `{_commands[0]}` command", slug="command-does-not-exist"
            )
