...
class AuthenticateTestSuite(AuthTestCase):
    ...
    def test__post__with_token__password_is_empty(self):
        ...
        for kwargs, token in cases:
            ...
            content = self.bc.format.from_bytes(response.content)
            expected = render_pick_password(self,
                                            'POST',
                                            token,
                                            data=data,
                                            messages=[Message('alert-danger', "Password can't be empty")])
                ...
                expected = render_pick_password(
                    ...
                    messages=[
                        Message('alert-danger',
                                'Password must contain 8 characters with lowercase, uppercase and symbols')
                    ])
    ...
    def test__post__with_token__right_password(self):
        ...
        cases = [({'user': user, 'token': token}, 'xyz'), ({'user': user, 'user_invite': user_invite}, 'abc')]
        for kwargs, token in cases:
            password_characters = (random.choices(string.ascii_lowercase, k=3) +
