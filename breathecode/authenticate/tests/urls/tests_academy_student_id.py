...
class AuthenticateTestSuite(AuthTestCase):
    ...
    def test_academy_student_id__passing_id__not_found(self):
        ...
        model = self.generate_models(authenticate=True, role=role, capability="read_student", profile_academy=True)
    ...
    def test_academy_student_id_with_github(self):
        ...
        role = "konan"
        model = self.generate_models(
            authenticate=True, role=role, capability="read_student", profile_academy=True, credentials_github=True
        )
        url = reverse_lazy("authenticate:academy_student_id", kwargs={"user_id_or_email": "1"})
        response = self.client.get(url)
        json = response.json()
    ...
    def test_academy_student_id__put__without_capability(self):
        ...
        self.assertEqual(
            json,
            {"detail": "You (user: 1) don't have this capability: crud_student " "for academy 1", "status_code": 403},
        )
        self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN)
    ...
    def test_academy_student_id__put__passing_email(self):
        ...
        role = "student"
        self.bc.request.set_headers(academy=1)
        model = self.generate_models(authenticate=True, role=role, capability="crud_student", profile_academy=True)
    ...
    def test_academy_student_id__put__without__first_name(self):
        ...
        json = response.json()
        expected = {"first_name": ["This field may not be blank."]}
    ...
    @patch("os.getenv", MagicMock(return_value="https://dotdotdotdotdot.dot"))
    def test_academy_student_id__put__without__last_name(self):
        """Test /academy/:id/member/:id"""
        profile_academy = {
            "first_name": self.bc.fake.first_name(),
            "last_name": self.bc.fake.last_name(),
        ...
        json = response.json()
        expected = {"last_name": ["This field may not be blank."]}
    ...
    def test_academy_student_id__put__without_required_fields(self):
        ...
        role = "crud_student"
        self.bc.request.set_headers(academy=1)
        model = self.generate_models(authenticate=True, role=role, capability="crud_student", profile_academy=1)
        url = reverse_lazy("authenticate:academy_student_id", kwargs={"user_id_or_email": "1"})
