...
def expected(monkeypatch: pytest.MonkeyPatch, fake: capy.Fake):
    ...
    monkeypatch.setattr(CapyAppProfileAcademySerializer, "get", m)
...
@pytest.fixture
def non_numeric_expected(expected):
    yield {
        **expected,
        "email": "a",
    }
...
def test_location_url(
    ...
    assert json == numeric_expected
    assert response.status_code == status.HTTP_200_OK


def test_location_url_(
    database: capy.Database,
    client: capy.Client,
    service: linked_services.Service,
    get_app_signature: Callable[[Any], dict[str, Any]],
    non_numeric_expected: dict[str, str],
):
    ...
    assert json == non_numeric_expected
    assert response.status_code == status.HTTP_200_OK
