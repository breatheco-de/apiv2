...
class AuthenticateTestSuite(AuthTestCase):
    ...
    def test_password_reset__post__without_data(self, mock):
        ...
        self.assertNotEqual(
            content.find('<title>Document</title>'),
            -1
        )
        self.assertNotEqual(
            content.find('Email is required'),
            -1
        )
        self.assertNotEqual(
            content.find('<label for="id_password1">Password1:</label></th><td><ul class="errorlist"><li>This field is required.</li>'),
            -1
        )
        self.assertNotEqual(
            content.find('<label for="id_password2">Password2:</label></th><td><ul class="errorlist"><li>This field is required.</li>'),
            -1
        )
    ...
    def test_password_reset__post__with_bad_passwords(self, mock):
        ...
        self.assertNotEqual(
            content.find('<label for="id_password1">Password1:</label></th><td><ul class="errorlist"><li>Ensure this value has at least 8 characters (it has 5).</li></ul>'),
            -1
        )
        self.assertNotEqual(
            content.find('<label for="id_password2">Password2:</label></th><td><ul class="errorlist"><li>Ensure this value has at least 8 characters (it has 5).</li></ul>'),
            -1
        )
    ...
    def test_password_reset__post__with_email(self, mock):
        ...
        self.assertNotEqual(
            content.find('Check your email for a password reset!'),
            -1
        )
    ...
    def test_password_reset__post__with_email__with_user(self, mock):
        ...
        response = self.client.post(url, data)
        content = response.content.decode('utf-8')
        token = Token.objects.filter(id=1).values_list('key', flat=True).first()
        ...
        self.assertEqual(mock.call_args_list, [call('pick_password', model['user'].email, {
