...
class MemberGetDuckTestSuite(AuthTestCase):
    ...
    def test_academy_id_member_id__with_auth(self):
        for n in range(1, 4):
            ...
            url = reverse_lazy(
                "authenticate:academy_id_member_id", kwargs={"academy_id": n, "user_id_or_email": f"{n}"}
            )
            response = self.client.get(url)
            json = response.json()
    ...
    def test_academy_id_member_id__with_auth___mock_view(self):
        model = self.bc.database.create(
            academy=3, capability="read_member", role="role", profile_academy=[{"academy_id": id} for id in range(1, 4)]
        )
