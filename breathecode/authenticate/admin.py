...
def sync_users_with_rigobot(modeladmin, request, queryset):
    ...
    for user in queryset:
        try:
            ...
            token, _ = Token.objects.get_or_create(user=user, token_type="login")

            # Sync with RigoBot
            async_to_sync(sync_with_rigobot)(token_key=token.key, organization="4geeks")
            success_count += 1
    ...
    if success_count > 0:
        messages.success(request, f"Successfully synced {success_count} user(s) with RigoBot.")

    if error_count > 0:
        messages.error(request, f"Failed to sync {error_count} user(s) with RigoBot. Check logs for details.")
