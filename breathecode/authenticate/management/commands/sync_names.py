    ...
    def handle(self, *args, **options):
        ...
        for profile in empty_profiles:
            if profile.first_name is None or profile.first_name == "":
                if profile.user is not None and profile.user.first_name is not None and profile.user.first_name != "":
                    save = True
                    profile.first_name = profile.user.first_name
            if profile.last_name is None or profile.last_name == "":
                if profile.user is not None and profile.user.last_name is not None and profile.user.last_name != "":
                    save = True
        ...
        for p in profiles:
            if p.user is not None and p.user.first_name is None or p.user.first_name == "":
                if p.first_name is not None and p.first_name != "":
                    save = True
                    p.user.first_name = p.first_name
            if p.user is not None and p.user.last_name is None or p.user.last_name == "":
