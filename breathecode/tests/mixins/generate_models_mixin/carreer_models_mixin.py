...
class CarreerModelsMixin(ModelsMixin):
    def generate_carreer_models(self,
                                platform=False,
                                position=False,
                                zyte_project=False,
                                spider=False,
        ...
        if not 'platform' in models and is_valid(platform):
            ...
            models['platform'] = create_models(platform, 'carreer.Platform', **{**kargs, **platform_kwargs})

        if not 'position' in models and (is_valid(position) or is_valid(spider)):
            kargs = {}

            models['position'] = create_models(position, 'carreer.Position', **{**kargs, **position_kwargs})
        ...
        if not 'zyte_project' in models and (is_valid(zyte_project) or is_valid(spider)):
            ...
            models['zyte_project'] = create_models(zyte_project, 'carreer.ZyteProject', **{
                **kargs,
                **zyte_project_kwargs
            })
        ...
        if not 'spider' in models and is_valid(spider):
            ...
            models['spider'] = create_models(spider, 'carreer.Spider', **{**kargs, **spider_kwargs})
        ...
        if not 'position_alias' in models and is_valid(position_alias):
            ...
            models['position_alias'] = create_models(position_alias, 'carreer.PositionAlias', **{
                **kargs,
                **position_alias_kwargs
            })

        if not 'carreer_tag' in models and is_valid(carreer_tag):
            kargs = {}

            models['carreer_tag'] = create_models(carreer_tag, 'carreer.CarreerTag', **{
                **kargs,
                **carreer_tag_kwargs
            })
        ...
        if not 'location' in models and is_valid(location):
            ...
            models['location'] = create_models(location, 'carreer.Location', **{**kargs, **location_kwargs})
        ...
        if not 'location_alias' in models and is_valid(location_alias):
            ...
            models['location_alias'] = create_models(location_alias, 'carreer.LocationAlias', **{
                **kargs,
                **location_alias_kwargs
            })
        ...
        if not 'employer' in models and is_valid(employer):
            ...
            models['employer'] = create_models(employer, 'carreer.Employer', **{**kargs, **employer_kwargs})
        ...
        if not 'job' in models and (is_valid(job) or is_valid(employer)):
            ...
            if 'carreer_tag' in models:
                kargs['carreer_tag'] = just_one(models['carreer_tag'])

            if 'location' in models:
                kargs['location'] = just_one(models['location'])

            models['job'] = create_models(job, 'carreer.Job', **{**kargs, **job_kwargs})
