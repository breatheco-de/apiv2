...
class SurveyTestSuite(FeedbackTestCase):
        ...
        expected = [
            {
                ...
                "cohort": {"id": model["cohort"].id, "slug": model["cohort"].slug, "name": model["cohort"].name},
    ...
    def test_academy_survey_delete_in_bulk_with_two_surveys(self):
        ...
        model = self.generate_models(
            authenticate=True, profile_academy=True, survey=2, capability="crud_survey", role=1
        )
    ...
    def test_academy_survey_id__delete__not_answered(self):
        ...
        for x in SURVEY_STATUS:
            answer = {"status": x}
            model = self.generate_models(
                user=1, profile_academy=True, survey=1, capability="crud_survey", role=1, answer=answer
            )
