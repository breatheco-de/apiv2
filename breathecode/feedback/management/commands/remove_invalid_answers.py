...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        for survey in Survey.objects.filter():
            ...
            pending_answers = Answer.objects.filter(survey=survey,
                                                    cohort=survey.cohort).exclude(status__in=['ANSWERED', 'OPENED'])
            ...
            for user_id in user_ids:
                ...
                if CohortUser.objects.filter(
                        user__id=user_id,
                        cohort=survey.cohort).exclude(educational_status__in=['ACTIVE', 'GRADUATED']).exists():
