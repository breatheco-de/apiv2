...
class RepositorySubscriptionSerializer(serializers.ModelSerializer):
    ...
    def validate(self, data):
        ...
        subs = RepositorySubscription.objects.filter(owner__id=academy_id,
                                                     repository=data['repository']).first()
        if subs is not None:
            raise ValidationException(
                translation(
                    ...
                    es=
                    'Ya existe una subscripcion para este mismo repositorio y owner, asegurate de tener accesso',
    ...
    def create(self, validated_data):
        ...
        try:
            _owner, _repo_name = instance.get_repo_name()
            gb = Github(org=settings.github_username, token=settings.github_owner.credentialsgithub.token)
            result = gb.subscribe_to_repo(_owner, _repo_name, instance.token)

            instance.hook_id = result['id']
            instance.save()
