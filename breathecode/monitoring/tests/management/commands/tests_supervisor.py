...
class TestIssueCleaner:
    ...
    def tests_older_issues_are_removed(self, database: dfx.Database, supervisor: Supervisor, patch, with_supervisor,
                                       with_issues, utc_now):
        extra = {}
        if with_supervisor:
            extra['supervisor'] = {
    ...
    def tests_recent_issues_keeps(self, bc: Breathecode, database: dfx.Database, supervisor: Supervisor, patch, utc_now,
                                  random: cfx.Random):
        model = database.create(supervisor={
            'delta': timedelta(seconds=3600),
            'ran_at': None,
            'task_module': 'breathecode.payments.supervisors',
            'task_name': 'supervise_all_consumption_sessions',
...
class TestSupervision:
    ...
    @pytest.mark.parametrize('supervised_at', [
        None,
        timedelta(days=2, seconds=1),
    ])
    def tests_pending_to_be_scheduled(self, database: dfx.Database, supervisor: Supervisor, patch, supervised_at,
                                      utc_now):
