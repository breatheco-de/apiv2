...
@pytest.mark.parametrize('task_module, task_name, error', [
    ('x', 'all_ok_supervisor', 'Module x not found'),
    ('breathecode.monitoring.tests.tasks.tests_run_supervisor', 'x',
...
def tests_supervision_with_issues(database: dfx.Database, supervisor: Supervisor, utc_now: datetime):
    ...
    assert supervisor.log('breathecode.monitoring.tests.tasks.tests_run_supervisor',
                          'something_went_wrong_supervisor') == [f'Something went wrong {x}' for x in range(3)]
    assert database.list_of('monitoring.Supervisor') == [
        db({
            'id': 1,
            'task_module': 'breathecode.monitoring.tests.tasks.tests_run_supervisor',
