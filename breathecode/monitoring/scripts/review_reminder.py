...
reviews = Review.objects.filter(status='PENDING', cohort__academy__id=4, cohort__ending_date__gte=FOUR_WEEKS_AGO, cohort__kickoff_date__lte=TODAY)
...
if total_reviews == 0:
    ...
    review_names = ('\n').join(['- ' + (r.author.first_name + " " + r.author.last_name + " for " + r.cohort.name + " in " + r.platform.name) for r in reviews])

    raise ScriptNotification(
        f'There are {str(total_reviews)} reviews to be requested: '
        f'\n {review_names}',
        status='FATAL',
