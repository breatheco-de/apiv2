...
3. **Load asset contract:** **`GET /v1/registry/asset/{associated_slug}`**. Read **`config`**, **`graded`**, **`asset_type`**, repo/readme URLs. Use [reference/learnpack-configuration.md](reference/learnpack-configuration.md) when interpreting **`config`** keys.
4. **Review** the student delivery (**`github_url`**, **`live_url`**, alignment with project expectations from **`config`**).
5. **Draft** teacher feedback for **`description`** — **max 450 characters** (API limit on **`Task.description`**).
6. **Submit:** **`PUT /v1/assignment/task/{task_id}`** with **`revision_status`** (**`APPROVED`**, **`REJECTED`**, **`PENDING`**, or **`IGNORED`**) and **`description`** as needed. Same endpoint for **ignoring** — see **Ignoring projects** under Edge Cases and Endpoints.
...
- **Query (discovery queue):** **`task_type=PROJECT`**, **`task_status=DONE`**, **`revision_status=PENDING`**, **`sort=delivered_at`** (mandatory for oldest-first), **`limit`**, **`offset`**. Optional: **`user_id`**, **`cohort`**, **`associated_slug`**.
...
- **Query:** Same discovery pattern: **`task_type=PROJECT`**, **`task_status=DONE`**, **`revision_status=PENDING`**, **`sort=delivered_at`**, **`limit`**, **`offset`**. Optional: **`student`**, **`educational_status`**, **`like`**, etc.
...
- **Query:** **`task_type=PROJECT`**, **`revision_status=PENDING`**, **`task_status=DONE`**, filters as needed. **No `sort` parameter** — server orders by **`created_at`** ascending (not **`delivered_at`** FIFO).
...
- **Response:** Asset payload including **`config`** (synced **`learn.json`**), **`asset_type`**, **`graded`**, URLs.
...
### 8. Ignore project (no separate ignore endpoint)
...
4. [ ] Feedback in **`description`** is **≤ 450 characters** before **`PUT`**.
5. [ ] Called **`PUT /v1/assignment/task/{task_id}`** with correct **`revision_status`** (including **`IGNORED`** only via this **`PUT`**, not a separate URL).
6. [ ] Did not override **`IGNORED`** from auto-ignore without explicit user instruction.
