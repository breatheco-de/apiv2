...
| **authenticate** | Staff login, academy-scoped capabilities, integration token, staff-assisted reset; staff invites and member admin; academy GitHub org user sync | `bc-authenticate-*`, [`bc-authenticate-staff-authentication`](../bc-authenticate-staff-authentication/SKILL.md), [`bc-authenticate-staff-invites`](../bc-authenticate-staff-invites/SKILL.md), [`bc-authenticate-manage-github-org-users`](../bc-authenticate-manage-github-org-users/SKILL.md) |
...
| Manage academy GitHub organization users (list, schedule ADD/DELETE/IGNORE, sync, invite URL, error log) | [`bc-authenticate-manage-github-org-users`](../bc-authenticate-manage-github-org-users/SKILL.md) + [`bc-authenticate-staff-authentication`](../bc-authenticate-staff-authentication/SKILL.md) |
| Rebuild a staff frontend for GitHub org membership sync | [`bc-authenticate-manage-github-org-users`](../bc-authenticate-manage-github-org-users/SKILL.md) + [`bc-authenticate-staff-authentication`](../bc-authenticate-staff-authentication/SKILL.md) |
...
| Create or update a registry asset (lesson, exercise, project, quiz) including category and technologies | [`bc-registry-manage-assets`](../bc-registry-manage-assets/SKILL.md) + [`bc-authenticate-staff-authentication`](../bc-authenticate-staff-authentication/SKILL.md) (`read_asset`/`crud_asset`, `Academy` header; look up category ids and technology slugs first) |
| Merge duplicate registry technology tags (parent/alias) and retag assets | [`bc-registry-manage-technologies`](../bc-registry-manage-technologies/SKILL.md) + [`bc-registry-manage-assets`](../bc-registry-manage-assets/SKILL.md) (PUT parent does not copy tags; retag each child `assets` slug with the parent) + [`bc-authenticate-staff-authentication`](../bc-authenticate-staff-authentication/SKILL.md) (`read_technology`/`crud_technology`, `Academy` header) |
| Create or edit an academy event with tags and workshop asset selection | `bc-events-create-and-edit-event` + `bc-marketing-*` (fetch valid `DISCOVERY` tags) + [`bc-registry-manage-assets`](../bc-registry-manage-assets/SKILL.md) (search and validate workshop assets by type via `GET /v1/registry/academy/asset`) |
...
| Configure event NPS, resend to non-responders, or review workshop satisfaction scores | [`bc-feedback-manage-event-surveys`](../bc-feedback-manage-event-surveys/SKILL.md) + [`bc-events-post-event`](../bc-events-post-event/SKILL.md) (finish event and attendance first) |
...
| Set up academy commerce end-to-end (payments → services → plans → courses) | [`bc-payments-manage-academy-payment-methods`](../bc-payments-manage-academy-payment-methods/SKILL.md) → `bc-payments-manage-services` → [`bc-payments-manage-plans`](../bc-payments-manage-plans/SKILL.md) → [`bc-marketing-create-or-clone-course`](../bc-marketing-create-or-clone-course/SKILL.md) |
| Create or update academy payment plans (sets, consumables, pricing, financing) | [`bc-payments-manage-plans`](../bc-payments-manage-plans/SKILL.md) |
| Configure plan upgrade / upsell offers between plans | [`bc-payments-manage-plans`](../bc-payments-manage-plans/SKILL.md) (Path B) |
| Read academy active-users / seat month invoice (daily snapshots + high-water-mark month) | [`bc-payments-read-active-users-bills`](../bc-payments-read-active-users-bills/SKILL.md) + [`bc-authenticate-staff-authentication`](../bc-authenticate-staff-authentication/SKILL.md) (`read_active_users_bill`, `Academy` header; `internal_billing` via Django admin) |
| Diagnose why an asset telemetry is missing for users/tasks | `bc-assignment-diagnose-asset-telemetry` + [`bc-registry-manage-assets`](../bc-registry-manage-assets/SKILL.md) (validate asset slug and translation/canonical context when telemetry appears split by locale) |
| Queue or interpret asset-level telemetry_stats (`telemetry_stats` JSON on assets) | `bc-assignment-diagnose-asset-telemetry` + [`bc-registry-manage-assets`](../bc-registry-manage-assets/SKILL.md) (read updated stats from the asset; `sync_telemetry_stats` is a registry action outside this skill’s write path) |
...
| Review **PROJECT** syllabus tasks (list pending **`DONE`** deliveries oldest-first, load asset **`config`**, submit **`PUT`** approval/rejection — **ignoring** uses the same **`PUT`** with **`revision_status=IGNORED`**, not a separate route) | [`bc-assignment-review-submit-task-revision`](../bc-assignment-review-submit-task-revision/SKILL.md) + [`bc-registry-manage-assets`](../bc-registry-manage-assets/SKILL.md) (academy asset GET for `config` and related fields) + [`bc-authenticate-staff-authentication`](../bc-authenticate-staff-authentication/SKILL.md) (token / headers as needed) |
