...
- **Completion / grading strategy**: Certificate eligibility uses `evaluate_cohort_user_completion(cohort_user)` from `breathecode/admissions/services/completion.py`. The evaluator reads `grading_strategy.completion` from the cohort syllabus and falls back to legacy mandatory PROJECT completion when no strategy exists. See `GRADING_STRATEGY_COMPLETION.md`.
...
4. **List specialties offered by the academy.** Call `GET /v1/certificate/academy/specialty` with `Authorization` and `Academy: <academy_id>`. The list includes **global** specialties (`academy_id` null — apply to all academies) and specialties owned by the academy. Specialties belonging to other academies are excluded. Response includes each specialty's id, slug, name, status, academy, syllabus (first of syllabuses), syllabuses. Use query `syllabus_slug=<slug>` to filter by syllabus, or `like=<text>` to search by name.
...
- **bad-educational-status / bad-finantial-status / cohort-without-status-ended / with-pending-tasks:** The student or cohort does not meet issuance rules. `with-pending-tasks-*` now refers to pending assets required by the active completion strategy, not only mandatory projects. Tell the user the exact slug or message; they must fix the student or cohort state before retrying.
